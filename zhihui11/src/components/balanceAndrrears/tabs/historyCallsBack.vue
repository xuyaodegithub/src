<template>
      <div class="history-calls-back">     <!-- -----历史话费返还 块-----  -->
        ...
        <div class="miscel-block-bottom">      <!--历史话费返还 表格 -->
          <div class="table-head">
            <div class="item-list-big">入账时间</div>
            <div class="item-list-big">返还计划</div>
            <div class="item-list">返还类型</div>
            <div class="item-list">入账金额</div>
            <div class="item-list">账本类型</div>
            <div class="item-list">失效时间</div>
          </div>
          <div class="table-body">
            <div class="table-list" v-for="item in queryBillReturnDetail">
              <div class="item-list-big">{{item.paymentDate}}</div>
              <el-tooltip  effect="dark" :content="item.promoName" placement="top">
                <div class="item-list-big" >{{item.promoName | wordLimit}}</div>
              </el-tooltip>
              <div class="item-list">{{item.backType}}</div>
              <div class="item-list">{{item.income}}</div>
              <div class="item-list">{{item.bookAccountName}}</div>
              <div class="item-list">{{item.expireDate}}</div>
            </div>
          </div>
...
<script>
  ...
  export default {
    ...
    created(){
//      let data = new Date();
//      this.lshf_date_month=data.getFullYear()+'-'+data.getMonth();
//      this.pre_select_history_contract=this.personMessage.billAccountNum;
    ...
    computed:{
      ...mapGetters([
        'assetVOS','personMessage','businessCallNumber','queryRechargeList','queryBillReturnDetail'
      ])
    },
    methods: {
      ...mapActions([
        //0000000000000
        'popoverAlert','queryBillReturnDetailActions'
      ]),
      search(){
        let data={
          'number':'',
