<template>
  <div class="history-contract-arrears">            <!-- -----历史合同欠费查询 块----- -------在其他块中---- -->
    ...
    <div class="miscel-block-bottom">      <!--历史合同欠费查询 表格 -->
      <div class="table-head">
        <div class="item-list">应用总额</div>
        <div class="item-list">欠费总额</div>
        <div class="item-list">违约金</div>
        <div class="item-list">违约金减免</div>
        <div class="item-list">上期零头</div>
        <div class="item-list">上月欠费</div>
      </div>
      <div class="table-body">
        <div class="table-list">
          <div class="item-list">{{queryOweChargeInfo.totalOweCharge}}</div>
          <div class="item-list">{{queryOweChargeInfo.totalOweFee}}</div>
          <div class="item-list">{{queryOweChargeInfo.totalLateFee}}</div>
          <div class="item-list">{{queryOweChargeInfo.totalLatefeeDerate}}</div>
          <div class="item-list">{{queryOweChargeInfo.change}}</div>
          <div class="item-list">{{queryOweChargeInfo.previousTotalOweFee}}</div>
        </div>
      </div>
...
<script>
  ...
  export default {
    data() {
      return {
        remind_sms_date_start:'',           //历史合同欠费查询日期 开始
        remind_sms_date_end:'',
...
//      let data = new Date();
//      this.remind_sms_date_start=data.getFullYear()+'-'+(data.getMonth()+1)+'-'+data.getDate();
//      this.remind_sms_date_end=data.getFullYear()+'-'+(data.getMonth()+1)+'-'+data.getDate();
//      this.pre_same_contract_number=this.personMessage.billAccountNum;
    ...
    computed:{
      ...mapGetters([
        'assetVOS','personMessage','businessCallNumber','queryOweChargeInfo'
      ])
    },
    methods: {
      ...mapActions([                                   //0000000000000
        'popoverAlert','queryOweChargeInfoActions'
      ]),
      search(){
        let data={
          'number':'',
