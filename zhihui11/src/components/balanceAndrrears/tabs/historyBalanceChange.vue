<template>
  <div class="history-balance-change">             <!-- -----历史余额变动 块----- -------在其他块中---- -->
    <div class="miscel-block-top">
      <span class="typeface lh40">开始月份</span>
      <div class="left10">
        <el-date-picker
      ...
      <span class="typeface lh40 distance15">结束月份</span>
      <div class="left10">
        <el-date-picker
    ...
    <div class="miscel-block-bottom">      <!--历史余额变动 表格 -->
      <div class="table-head">
        <div class="item-list">时间</div>
        <div class="item-list">上期末余额</div>
        <div class="item-list">收入</div>
        <div class="item-list">支出</div>
        <div class="item-list">期末余额</div>
      </div>
      <div class="table-body">
        <div class="table-list" v-for="item in queryBalanceLogTotal">
          <div class="item-list"> {{item.month}}</div>
          <div class="item-list">{{item.preBalance}}</div>
          <div class="item-list">{{item.curIncome}}</div>
          <div class="item-list">{{item.curExpenditure}}</div>
          <div class="item-list">{{item.curBalance}}</div>
        </div>
      </div>
...
<script>
  ...
  export default {
    data() {
      return {
        remind_sms_date_start:'',     //历史余额变动 月 开始
        remind_sms_date_end:'',
...
//      let data = new Date();
//      this.remind_sms_date_start=data.getFullYear()+'-'+(data.getMonth()+1);
//      this.remind_sms_date_end=data.getFullYear()+'-'+(data.getMonth()+1);
//      this.pre_same_contract_number=this.personMessage.billAccountNum;
    ...
    computed:{
      ...mapGetters([
        'assetVOS','personMessage','businessCallNumber','queryBalanceLogTotal'
      ])
    },
    methods: {
      ...mapActions([                                   //0000000000000
        'popoverAlert','queryBalanceLogTotalActions'
      ]),
      search(){
        let data={
          'number':'',
        ...
        if(typeof this.remind_sms_date_start == 'string'){
          ...
          data.startDate=this.remind_sms_date_start.split('-')[0]+mouth;
        }else{
          let mouth = '';
          let day = '';
          if((this.remind_sms_date_start.getMonth()+1)<10){
            mouth = '0'+(this.remind_sms_date_start.getMonth()+1);
          }else{
            mouth = (this.remind_sms_date_start.getMonth()+1);
          }
          ...
          data.startDate=this.remind_sms_date_start.getFullYear()+mouth;
        }
        if(typeof this.remind_sms_date_end == 'string'){
          ...
          data.endDate=this.remind_sms_date_end.split('-')[0]+mouth;
        }else{
          let mouth = '';
          let day = '';
          if((this.remind_sms_date_end.getMonth()+1)<10){
            mouth = '0'+(this.remind_sms_date_end.getMonth()+1);
          }else{
            mouth = (this.remind_sms_date_end.getMonth()+1);
          }
          ...
          data.endDate=this.remind_sms_date_end.getFullYear()+mouth;
        }
