 ...
 *               in log4j for Java. The API should be nearly the same. This file
 *               contains all log4js code and is the only file required for
 *               logging.
 *               <h3>Example:</h3>
 * 
 * <pre>
 ...
 * 
 * for (index = 0; index < documentScripts.length; ++index) { var documentScript =
 * documentScripts[index]; if (documentScript.src == src) { return false; } }
 * 
...
Log4js.Level.prototype =
{
     ...
     * @param {Log4js.Level} defaultLevel default Level, if no String
     *            representation
     * @return Level object
     * @type Log4js.Level
     */
...
Log4js.Logger.prototype =
{
    ...
    setAppenders : function(appenders)
    {
        ...
        for ( var i = 0; i < this.appenders.length; i++)
        {
            ...
            // 2011/06/03 moidfy by QianRongHua
            this.onlog.removeListener(Log4js.bind(this.doAppend, this));
        	this.onclear.removeListener(Log4js.bind(this.doClear, this));
        }
     ...
     * 2011/06/02 modify by QianRongHua Limit memory growth ArrayLength<500
     * 
     * @private
     */
    log : function(logLevel, message, exception)
    {
        ...
        if(this.loggingEvents.length>maxLog)
        {
        	this.loggingEvents.shift();
        }
 ...
 * @param {boolean} isInline boolean value that indicates whether the console be
 *            placed inline, default is to launch in new window
...
Log4js.ConsoleAppender.prototype = Log4js
        .extend(
                new Log4js.Appender(),
                {

                    /**
                     * Set the access key to show/hide the inline console
                     * (default &quote;d&quote;)
                     * 
                    ...
                    initialize : function()
                    {
                        ...
                        // I hate writing javascript in HTML... but what's a
                        // better alternative
                        ...
                        if (this.inline)
                        {
                            ...
                            closeButton.style.styleFloat = "right"; // IE dom
                                                                    // bug...doesn't
                                                                    // understand
                                                                    // cssFloat
                        ...
                        clearButton.style.styleFloat = "right"; // IE dom
                                                                // bug...doesn't
                                                                // understand
                                                                // cssFloat
                        ...
                        this.tagFilterContainerElement.appendChild(this.docReference.createTextNode("Log4js - "
                        ...
                        this.tagFilterElement.setAttribute('autocomplete', 'off'); // So
                                                                                    // Firefox
                                                                                    // doesn't
                                                                                    // flip
                                                                                    // out
                        ...
                        this.inputElement.style.borderWidth = '0px'; // Inputs
                                                                        // with
                                                                        // 100%
                                                                        // width
                                                                        // always
                        ...
                        this.inputElement.setAttribute('autocomplete', 'off'); // So
                                                                                // Firefox
                                                                                // doesn't
                                                                                // flip
                                                                                // out
                    ...
                    show : function()
                    {
                        ...
                        this.outputElement.scrollTop = this.outputElement.scrollHeight; // Scroll
                                                                                        // to
                                                                                        // bottom
                                                                                        // when
                                                                                        // toggled
                    ...
                    output : function(message, style)
                    {
                        ...
                        // If we are at the bottom of the window, then keep
                        // scrolling with the output
                    ...
                    {
                        var offset = window.pageYOffset || this.docReference.documentElement.scrollTop
...
Log4js.AjaxAppender.prototype = Log4js
        .extend(
                new Log4js.Appender(),
                {
                    /**
                    ...
                    doAppend : function(loggingEvent)
                    {
                        ...
                        {
                            // if threshold is reached send the events and reset
                            // current threshold
                            this.send();
                        }
                    ...
                    /**
                     * Set the threshold when logs have to be send. Default
                     * threshold is 1.
                     * 
                    ...
                    /**
                     * Set the timeout in milli seconds until sending request is
                     * aborted. Default is 2000 ms.
                     * 
                    ...
                    send : function()
                    {
                        if (this.loggingEventMap.length() > 0)
                        {
                            ...
                            // URI which may differ from the location of the
                            // error if
                    ...
                    onReadyStateChanged : function()
                    {
                        ...
                        {
                            log4jsLogger.trace("< AjaxAppender.onReadyStateChanged: readyState " + req.readyState
                                    + " != 4");
                            return;
                        }
                        ...
                        else
                        {
                            var msg = "  AjaxAppender.onReadyStateChanged: XMLHttpRequest request to URL "
                                    + this.loggingUrl + " returned status code " + this.httpRequest.status;
...
Log4js.WindowsEventAppender.prototype = Log4js.extend(new Log4js.Appender(),
{
    ...
    doAppend : function(loggingEvent)
    {
        ...
        // Windows events: - SUCCESS: 0, ERROR: 1, WARNING: 2, INFORMATION: 4,
        // AUDIT_SUCCESS: 8, AUDIT_FAILURE: 16
 ...
 * <pre>
 * categoryName
 * &tilde;startTime[logLevel]message\n
 * </pre>
 * 
 * @constructor
 * @extends Log4js.Layout
            ...
            else
            {
                ...
                switch (conversionCharacter)
                {
                    ...
                    case "p":
                        ...
                        break;
                    case "r":
                        replacement = "" + loggingEvent.startTime.toLocaleTimeString(); // TODO:
                                                                                        // .getDifference(Log4js.applicationStartDate);
                        break;
                    case "%":
                        replacement = "%";
                        break;
...
if (!Array.prototype.push)
{
     ...
     * few functions. More info at {@link http:// prototype.conio.net/}
     * 
     * @private
     */
